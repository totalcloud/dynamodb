...
Scan.prototype.attributes = function(attrs) {
  ...
  var expressionAttributeNames = _.reduce(attrs, function (result, attr) {
    ...
    complexAttrs.push(split.map(s => `#${s}`).join('.'));

    return result;
  }, {});

  this.request.ProjectionExpression = _.keys(expressionAttributeNames)
    .concat(complexAttrs)
    .join(',');
  this.request.ExpressionAttributeNames = _.merge({}, expressionAttributeNames, complexVal, this.request.ExpressionAttributeNames);

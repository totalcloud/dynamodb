...
exports.buildFilterExpression = function (key, operator, existingValueNames, val1, val2 ) {
  ...
  const isDeepKey = key.indexOf('.') > -1;
  var path = !isDeepKey ? '#' + key
    : key.split('.').map(e => `#${e}`).join('.');
  var valueName = !isDeepKey ? key : key.split('.').join('_');
  var v1ValueName = internals.uniqAttributeValueName(valueName, existingValueNames);
  var v2ValueName = internals.uniqAttributeValueName(valueName, [v1ValueName].concat(existingValueNames));
...
internals.buildInFilterExpression = function (key, existingValueNames, values) {
  ...
  var attributeValues = _.reduce(values, function(result, val) {
    ...
    var valueName = !isDeepKey ? key : key.split('.').join('_');
    var p = internals.uniqAttributeValueName(valueName, existing);
